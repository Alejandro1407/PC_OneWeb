...
$(document).ready(function(){
   PrintMenu();
    ...
    $('#RegisterFormTelefono').on("keyup", function () {
        CheckRegisterTelefono(0);
    });
    $('#RegisterFormTelefono').on("focusout", function () {
        CheckRegisterTelefono(1);
    });
...
function CheckRegisterNombre(Isfocusout) { 
    ...
    if(Check === true){
        ...
        if ((RegisterValidNombre === 1) && (RegisterValidEmail === 1) && (RegisterValidContraseña === 1) && (RegisterValidTelefono === 1)) {
...
function CheckRegisterTelefono(Isfocusout) {
    var Check_Telefono = /^(22|78|60|61|73){1}[0-9]{6}$/g;
    var Telefono = $('#RegisterFormTelefono').val();
    var Check = Check_Telefono.test(Telefono);
    if (Check === true) { //Debe ser igualada o genera Bug 
        RegisterValidTelefono = 1;
        $('#RegisterFormTelefonoInValid').addClass("d-none");
        $('#RegisterFormTelefonoValid').removeClass("d-none");
        if ((RegisterValidNombre === 1) && (RegisterValidEmail === 1) && (RegisterValidContraseña === 1) && (RegisterValidTelefono === 1)) {
            $('#RegisterFormBtn').removeAttr("disabled", "disabled");
        } else {
            $('#RegisterFormBtn').attr("disabled", "disabled");
        ...
        $('#RegisterFormBtn').attr("disabled", "disabled");
        RegisterValidTelefono = 0;
        $('#RegisterFormTelefonoValid').addClass("d-none");
        if (Isfocusout === 1) {
            $('#RegisterFormTelefonoValid').addClass("d-none");
            $('#RegisterFormTelefonoInValid').removeClass("d-none");
        }
    }
    ...
    var Telefono = $('#RegisterFormTelefono').val();
       $.ajax({
        type:"POST", //Tipo de peticion
           ...
           data: { email: Email, usuario: Nombre, contrasena: Contraseña, telefono: Telefono },
        beforeSend:function(){
...
function MostrarCarrito(){
    ...
    let to_print = '<button  onclick="CheckOut()" class="btn btn-white ml-auto d-block mr-5 p-2 shadow-none Font-Family-Lato"><i class="far fa-2x fa-credit-card"></i> CheckOut</button>';
           for(let i = 0; i < Articulos.length;i++){
            ...
            to_print += '<p class="text-muted text-center">'+Articulos[i].Categoria+'</p>';
            to_print += '<p class="text-muted text-center">Precio:$'+Articulos[i].Precio+'</p></div>';
